...
def save_intrinsic_as_json(filename, frame):
    ...
    # extrinsics = frame.profile.as_video_stream_profile().extrinsics
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_folder",
                        default='./dataset'+str(time.time())+'/realsense/',
                        # default='./dataset/realsense/',
                        help="set output folder")
    parser.add_argument("--record_rosbag",
                        action='store_true',
                        help="Recording rgbd stream into realsense.bag",default=False)
    parser.add_argument(
        "--record_imgs",
        action='store_true',
        help="Recording save color and depth images into realsense folder",default=True)
    ...
    try:
        while True:
            ...
            if args.record_imgs:
                ...
                cv2.imwrite(path_depth+r'/'+str(frame_count).zfill(6)+'-'+str(time.time())+'.png',
                            depth_image)
                # cv2.imwrite("%s/%06d.jpg" % \
                #         (path_color, frame_count), color_image)
                cv2.imwrite(path_color+r'/'+str(frame_count).zfill(6)+'-'+str(time.time())+'.png',
                            color_image)
                # np.save(path_depth+r'/'+str(frame_count).zfill(6)+'-'+str(time.time())+'.npy',
                #              depth_image)
                # np.save(path_color+r'/'+str(frame_count).zfill(6)+'-'+str(time.time())+'.npy',
                #             color_image)
            ...
            # cv2.resizeWindow('Recorder Realsense',1280,360)
            #
